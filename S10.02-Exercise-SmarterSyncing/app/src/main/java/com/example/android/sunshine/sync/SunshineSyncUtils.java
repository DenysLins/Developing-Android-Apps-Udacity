...
public class SunshineSyncUtils {
    ...
    private static boolean sEmpty = true;
    ...
    synchronized public static void initialize(Context context) {
        ...
        if (!sInitialized) {
            ...
            CheckProviderIsEmpty checkProviderIsEmpty = new CheckProviderIsEmpty();
            checkProviderIsEmpty.execute(context);
            //  Completed (6) If it is empty or we have a null Cursor, sync the weather now!
            if (sEmpty) startImmediateSync(context);
        };
    }
    ...
    private static class CheckProviderIsEmpty extends AsyncTask<Context, Void, Boolean> {

        protected Boolean doInBackground(Context... context) {

            boolean result;

            ContentResolver sunshineContentResolver = context[0].getContentResolver();
