...
//      Completed (9) Handle queries on both the weather and weather with date URI
        SQLiteDatabase wheatherDatabase = mOpenHelper.getReadableDatabase();
        int match = uriMatcher.match(uri);
        Cursor weatherCursor;

        switch (match) {
            case CODE_WEATHER:
                weatherCursor = wheatherDatabase.query(WeatherContract.WeatherEntry.TABLE_NAME,
                        projection,
                        selection,
                        selectionArgs,
                        null,
            ...
            case CODE_WEATHER_WITH_DATE:
                ...
                weatherCursor = mOpenHelper.getReadableDatabase().query(
...
//      Completed (10) Call setNotificationUri on the cursor and then return the cursor
        weatherCursor.setNotificationUri(getContext().getContentResolver(), uri);
