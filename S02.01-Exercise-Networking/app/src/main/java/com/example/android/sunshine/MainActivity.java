...
//        }
    ...
    private void loadWeatherData() {
        ...
        URL url = NetworkUtils.buildUrl(locale.getDisplayCountry());

        new WheatherDataQueryTask().execute(url);
    }

    // TODO (5) Create a class that extends AsyncTask to perform network requests
    // TODO (6) Override the doInBackground method to perform your network requests
    // TODO (7) Override the onPostExecute method to display the results of the network request
    private class WheatherDataQueryTask extends AsyncTask<URL, Void, String> {
        String wheatherDataSearchResults = null;

        @Override
        protected String doInBackground(URL... params) {
            try {
                wheatherDataSearchResults = NetworkUtils.getResponseFromHttpUrl(params[0]);
            } catch (IOException e) {
                e.printStackTrace();
            }
            return null;
        ...
        protected void onPostExecute(String s) {
            mWeatherTextView.setText(wheatherDataSearchResults);
        }
    }
}