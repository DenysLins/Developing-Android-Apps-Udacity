...
//          Completed (2) Only perform our implementation of bulkInsert if the URI matches the CODE_WEATHER code
        final SQLiteDatabase weatherDatabase = mOpenHelper.getWritableDatabase();
            ...
            case CODE_WEATHER:
                ...
                try {
                    for (ContentValues value : values) {
                        long data = value.getAsLong(WeatherContract.WeatherEntry.COLUMN_DATE);
                        if (!(SunshineDateUtils.isDateNormalized(data)))
                            throw new IllegalArgumentException("Date not normalized");
                        long _id = weatherDatabase.insert(WeatherContract.WeatherEntry.TABLE_NAME,
                                null,
                                value);
...
//              completed (3) Return the number of rows inserted from our implementation of bulkInsert
                if (numInserted > 0) getContext().getContentResolver().notifyChange(uri, null);
